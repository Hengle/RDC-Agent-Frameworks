...
$Specs = @(
 @{ Key = "claude-code"; ManagedDirs = @(".claude", "common"); ManagedFiles = @("README.md") },
 @{ Key = "code-buddy"; ManagedDirs = @(".codebuddy-plugin", "agents", "skills", "hooks", "common"); ManagedFiles = @("README.md", ".mcp.json") },
 @{ Key = "copilot-cli"; ManagedDirs = @("agents", "skills", "hooks", "common"); ManagedFiles = @("README.md", ".mcp.json", ".copilot-plugin.json") },
 @{ Key = "copilot-ide"; ManagedDirs = @(".github", "references", "common"); ManagedFiles = @("README.md", "agent-plugin.json") },
 @{ Key = "claude-desktop"; ManagedDirs = @("references", "common"); ManagedFiles = @("README.md", "claude_desktop_config.json") },
 @{ Key = "manus"; ManagedDirs = @("references", "workflows", "common"); ManagedFiles = @("README.md") },
 @{ Key = "codex"; ManagedDirs = @(".agents", ".codex", "common"); ManagedFiles = @("README.md", "AGENTS.md") }
)
...
function Agent-Body([string]$PlatformKey, $Role, [string]$TargetFile) { $rolePath = Join-Path (Common-Root $PlatformKey) $Role.source_prompt; $p1 = Common-Ref $PlatformKey $TargetFile @("AGENT_CORE.md"); $p2 = Rel-Path $TargetFile $rolePath; $p3 = Common-Ref $PlatformKey $TargetFile @("skills", "renderdoc-rdc-gpu-debug", "SKILL.md")
@"
@"
...
function CodeBuddy-Agent($Role, [string]$TargetFile) {
 ...
 return ($front + "`n`n" + (Agent-Body "code-buddy" $Role $TargetFile))
}

function ClaudeCode-Agent($Role, [string]$TargetFile) {
 $front = Yaml-Block ([ordered]@{ description = $Role.description; model = (Platform-Model "claude-code" $Role.agent_id) })
 return ($front + "`n`n" + (Agent-Body "claude-code" $Role $TargetFile))
}

function CopilotIde-Agent($Role, [string]$TargetFile) {
 ...
 return ($front + "`n`n" + (Agent-Body "copilot-ide" $Role $TargetFile))
}

function CopilotCli-Agent($Role, [string]$TargetFile) {
 $front = Yaml-Block ([ordered]@{ description = $Role.description })
 return ($front + "`n`n" + (Agent-Body "copilot-cli" $Role $TargetFile))
}

function Skill-Wrapper([string]$PlatformKey, [string]$TargetFile) { $skillRef = Common-Ref $PlatformKey $TargetFile @("skills", "renderdoc-rdc-gpu-debug", "SKILL.md"); $capRef = Common-Ref $PlatformKey $TargetFile @("config", "platform_capabilities.json")
@"
...
function ClaudeCode-Entry([string]$TargetFile) { $p1 = Common-Ref "claude-code" $TargetFile @("AGENT_CORE.md"); $p2 = Common-Ref "claude-code" $TargetFile @("skills", "renderdoc-rdc-gpu-debug", "SKILL.md"); $p3 = Common-Ref "claude-code" $TargetFile @("docs", "platform-capability-model.md")
@"
# Claude Code Entry
...
function Copilot-Instructions([string]$TargetFile) { $p1 = Common-Ref "copilot-ide" $TargetFile @("AGENT_CORE.md"); $p2 = Common-Ref "copilot-ide" $TargetFile @("skills", "renderdoc-rdc-gpu-debug", "SKILL.md"); $p3 = Common-Ref "copilot-ide" $TargetFile @("docs", "platform-capability-model.md")
@"
...
function References-Entry([string]$PlatformKey, [string]$TargetFile) { $p1 = Common-Ref $PlatformKey $TargetFile @("AGENT_CORE.md"); $p2 = Common-Ref $PlatformKey $TargetFile @("docs", "platform-capability-model.md"); $p3 = Common-Ref $PlatformKey $TargetFile @("skills", "renderdoc-rdc-gpu-debug", "SKILL.md")
@"
...
function ClaudeDesktop-Config() {
 $servers = @{}
 foreach ($prop in $McpServers.servers.PSObject.Properties) { $servers[$prop.Name] = @{ command = $prop.Value.command; args = @($prop.Value.args) } }
 return @{ mcpServers = $servers }
}
function Codex-Readme() {
@"
# Codex Template
...
function Codex-AgentsMd([string]$TargetFile) { $p1 = Common-Ref "codex" $TargetFile @("AGENT_CORE.md"); $p2 = Common-Ref "codex" $TargetFile @("skills", "renderdoc-rdc-gpu-debug", "SKILL.md"); $p3 = Common-Ref "codex" $TargetFile @("docs", "platform-capability-model.md"); $p4 = Common-Ref "codex" $TargetFile @("docs", "model-routing.md")
@"
...
function Codex-RoleConfig($Role, [string]$TargetFile) {
...
function Expected-Files($Spec) {
 ...
 $expected = @{}
 foreach ($entry in (Common-Placeholder-Files $Spec.Key).GetEnumerator()) { $expected[$entry.Key] = $entry.Value }
 if ($Spec.Key -eq "codex") { Add-Expected $expected (Join-Path $package "README.md") (Codex-Readme) } else { Add-Expected $expected (Join-Path $package "README.md") (Readme $Spec.Key) }
 if (@("claude-code", "code-buddy", "copilot-cli", "copilot-ide") -contains $Spec.Key) {
 foreach ($role in (Roles)) {
 $fileName = $role.platform_files.($Spec.Key)
 if (-not $fileName) { continue }
 if ($Spec.Key -eq "claude-code") { $target = Join-Path $package (Join-Path ".claude\agents" $fileName); Add-Expected $expected $target (ClaudeCode-Agent $role $target) }
 elseif ($Spec.Key -eq "code-buddy") { $target = Join-Path $package (Join-Path "agents" $fileName); Add-Expected $expected $target (CodeBuddy-Agent $role $target) }
 elseif ($Spec.Key -eq "copilot-cli") { $target = Join-Path $package (Join-Path "agents" $fileName); Add-Expected $expected $target (CopilotCli-Agent $role $target) }
 elseif ($Spec.Key -eq "copilot-ide") { $target = Join-Path $package (Join-Path ".github\agents" $fileName); Add-Expected $expected $target (CopilotIde-Agent $role $target) }
 }
 }
 if ($Spec.Key -eq "code-buddy") {
 $skill = Join-Path $package "skills\renderdoc-rdc-gpu-debug\SKILL.md"
 Add-Expected $expected $skill (Skill-Wrapper $Spec.Key $skill)
 Add-Expected $expected (Join-Path $package ".codebuddy-plugin\plugin.json") (ConvertTo-Json (CodeBuddy-Plugin) -Depth 20)
 Add-Expected $expected (Join-Path $package ".mcp.json") (ConvertTo-Json (Mcp-Payload) -Depth 20)
 Add-Expected $expected (Join-Path $package "hooks\hooks.json") (ConvertTo-Json (CodeBuddy-Hooks) -Depth 20)
 } elseif ($Spec.Key -eq "copilot-cli") {
 $skill = Join-Path $package "skills\renderdoc-rdc-gpu-debug\SKILL.md"
 Add-Expected $expected $skill (Skill-Wrapper $Spec.Key $skill)
 Add-Expected $expected (Join-Path $package ".copilot-plugin.json") (ConvertTo-Json (CopilotCli-Plugin) -Depth 20)
 Add-Expected $expected (Join-Path $package ".mcp.json") (ConvertTo-Json (Mcp-Payload) -Depth 20)
 Add-Expected $expected (Join-Path $package "hooks\hooks.json") (ConvertTo-Json (CopilotCli-Hooks) -Depth 20)
 } elseif ($Spec.Key -eq "claude-code") {
 $entry = Join-Path $package ".claude\CLAUDE.md"
 Add-Expected $expected $entry (ClaudeCode-Entry $entry)
 Add-Expected $expected (Join-Path $package ".claude\settings.json") (ConvertTo-Json (ClaudeCode-Settings) -Depth 20)
 ...
 Add-Expected $expected $skill (Skill-Wrapper $Spec.Key $skill)
 Add-Expected $expected $entry (Copilot-Instructions $entry)
 Add-Expected $expected $ref (References-Entry $Spec.Key $ref)
 Add-Expected $expected (Join-Path $package "agent-plugin.json") (ConvertTo-Json (CopilotIde-Plugin) -Depth 20)
 ...
 $ref = Join-Path $package "references\entrypoints.md"
 Add-Expected $expected $ref (References-Entry $Spec.Key $ref)
 Add-Expected $expected (Join-Path $package "claude_desktop_config.json") (ConvertTo-Json (ClaudeDesktop-Config) -Depth 20)
 } elseif ($Spec.Key -eq "manus") {
 $ref = Join-Path $package "references\entrypoints.md"
 Add-Expected $expected $ref (References-Entry $Spec.Key $ref)
 Add-Expected $expected (Join-Path $package "workflows\00_debug_workflow.md") (Manus-Workflow)
 ...
 Add-Expected $expected $entry (Codex-AgentsMd $entry)
 Add-Expected $expected $skill (Skill-Wrapper $Spec.Key $skill)
 Add-Expected $expected (Join-Path $package ".codex\config.toml") (Codex-Config)
 foreach ($role in (Roles)) {
 $key = $role.platform_files.codex
 $target = Join-Path $package (Join-Path ".codex\agents" "$key.toml")
 Add-Expected $expected $target (Codex-RoleConfig $role $target)
 }
 }
 return $expected
}
function Compare-Files($Expected) {
 ...
 $current = Normalize (Get-Content $path -Raw)
