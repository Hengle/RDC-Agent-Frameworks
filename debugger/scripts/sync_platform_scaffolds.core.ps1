...
function Readme([string]$PlatformKey) {
 ...
 ('4. 运行 {0}，确认 {1}、snapshot、宿主入口文件与共享文档全部对齐。' -f (Code 'python common/config/validate_binding.py --strict'), (Code 'tools_root')),
 ('5. 正式发起 debug 前，用户必须在当前对话提交至少一份 {0}。' -f (Code '.rdc')),
 ('6. 使用当前平台根目录同级的 {0} 作为运行区。' -f (Code 'workspace/')),
 "7. 完成覆盖后，再在对应宿主中打开当前平台根目录。",
 ('8. 正常用户请求从 {0} 发起；其他 specialist 默认是 internal/debug-only。' -f (Code 'team_lead')),
 "",
 "约束：",
 "",
 ...
 ('- 当前工具 snapshot 必须与 {0} 的 {1} 个 tools 对齐，并包含新增的只读 {2} 探索面与统一 tabular projection 能力。' -f (Code 'RDC-Agent-Tools'), (Code '202'), (Code 'rd.vfs.*')),
...
function CodexReadme() {
 $lines = @(
  ...
  ('- 默认入口是 daemon-backed {0}；当前宿主的 `CLI` 与 `MCP` 都依赖同一 daemon-owned runtime / context。' -f (Code 'CLI')),
  ...
  '',
 ('1. 将仓库根目录 {0} 整体拷贝到当前平台根目录的 {1}，覆盖占位内容。' -f (Code 'debugger/common/'), (Code 'common/')),
 ('2. 在复制后的平台包根目录 {0} 中配置 {1}；JSON 文件中的 Windows 路径必须写成前斜杠或转义反斜杠。' -f (Code 'common/config/platform_adapter.json'), (Code 'paths.tools_root')),
 ('3. 确认 {0} 在 {1} 下全部存在。' -f (Code 'validation.required_paths'), (Code '<resolved tools_root>/')),
 ('4. 运行 {0}，确认 {1}、snapshot、宿主入口文件与共享文档全部对齐。' -f (Code 'python common/config/validate_binding.py --strict'), (Code 'tools_root')),
 ('5. 正式发起 debug 前，用户必须在当前对话提交至少一份 {0}。' -f (Code '.rdc')),
 ('6. 使用当前平台根目录同级的 {0} 作为运行区。' -f (Code 'workspace/')),
  ...
  ('8. 正常用户请求从 {0} 发起；其他 specialist 默认是 internal/debug-only。' -f (Code 'team_lead')),
  ('9. {0}、{1}、{2} 与 {3} 只允许引用当前平台根目录的 common/。' -f (Code 'AGENTS.md'), (Code '.agents/skills/'), (Code '.codex/config.toml'), (Code '.codex/agents/*.toml')),
  '',
  '约束：',
  '',
  ('- {0} 默认只保留一个占位文件；正式共享正文仍由顶层 {1} 提供，并由用户显式拷入。' -f (Code 'common/'), (Code 'debugger/common/')),
