...
若发现 taxonomy / SOP / invariant 需要调整，只能在 `common/knowledge/proposals/` 生成 candidate，并交给自动 replay / shadow / promotion 流程处理。
...
Step 4c: Candidate 发射（若本次调试发现 SOP / invariant / taxonomy 有缺漏）
  → 在 BugFull 的 knowledge_evolution_notes 字段记录建议
  → 发射 candidate，进入 replay / shadow / auto-promotion
...
- ❌ 绕过 candidate / registry / manifest 流程直接改写 active spec
